...
function downloadElliotPDF() {
    // Télécharger le fichier ODT depuis GitHub Pages
    const url = 'https://aazyris.github.io/elliot/RapportE.odt';
    const filename = 'RapportE.odt';
...
function downloadMirfayelPDF() {
    // Télécharger le PDF depuis GitHub Pages
    const url = 'https://aazyris.github.io/mirfayel/Animation%20p%C3%A9riscolaire.pdf';
    const filename = 'Animation périscolaire.pdf';
...
function downloadExistingFile(filename) {
    const a = document.createElement('a');
    a.href = filename;
    a.download = filename.split('/').pop(); // Extraire juste le nom du fichier
    a.target = '_blank';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
